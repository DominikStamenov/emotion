...
export function createRibbonEngine({
  ...
  function update(
    ...
    positions.forEach((strandPositions, strandIndex) => {
      ...
      for (let pointIndex = 0; pointIndex < pointsPerStrand; pointIndex += 1) {
         ...
         */
        const swirlStrength =
          (1 - progress) * (1 - progress) * 0.52 * direction;

        const animatedAngle =
          angle +
          swirlStrength +
          Math.sin(
            elapsedTime * 0.58 + strandPhase + progress * Math.PI * 1.6,
          ) *
            0.11 *
            envelope;
        ...
        const secondaryFlow =
          Math.sin(
            progress * Math.PI * 5.1 + elapsedTime * 0.7 + strandPhase * 1.4,
          ) *
          0.09 *
          envelope;
        ...
        let x = baseX + normalX * (flow + secondaryFlow + strandOffset);

        let y = baseY + normalY * (flow + secondaryFlow + strandOffset);

        let z =
          Math.sin(
            progress * Math.PI * 2.2 + elapsedTime * 0.31 + strandPhase,
          ) *
          0.3 *
          envelope;
        ...
        if (
          ...
          const repulsion = influence * influence * (0.32 + safeDelta * 1.8);
          ...
          z +=
            Math.sin(
              elapsedTime * 5.2 - distance * 5.4 + progress * Math.PI * 3,
            ) *
            influence *
            0.28;
        }
