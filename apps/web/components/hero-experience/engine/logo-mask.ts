export type LogoMaskTargets = {
    ...
    return new Promise<HTMLImageElement>(
      (resolve, reject) => {
        ...
        image.onerror = () => {
          reject(
            new Error(
              `Unable to load logo source: ${src}`,
            ),
          );
        };
  ...
  export async function loadLogoMaskTargets({
    ...
    if (!context) {
      throw new Error(
        "Unable to create logo mask canvas context.",
      );
    }
    ...
    if (maxX < minX || maxY < minY) {
      throw new Error(
        `No visible pixels detected in ${src}.`,
      );
    }
    ...
    if (allCandidates.length === 0) {
      throw new Error(
        `Logo source ${src} contains no usable pixels.`,
      );
    }
    ...
    const halfExtent =
      Math.max(activeWidth, activeHeight) * 0.5;
    ...
    for (
      ...
      const pixelY =
        Math.floor(pixelIndex / resolution);
      ...
      const sourceColorOffset =
        pixelIndex * 4;
  
      const targetColorOffset =
        index * 3;
      ...
      colors[targetColorOffset] = Math.min(
        1,
        ((pixelData[sourceColorOffset] ?? 255) /
          255) *
          edgeGain,
      );
  
      colors[targetColorOffset + 1] =
        Math.min(
          1,
          ((pixelData[sourceColorOffset + 1] ??
            255) /
            255) *
            edgeGain,
        );
  
      colors[targetColorOffset + 2] =
        Math.min(
          1,
          ((pixelData[sourceColorOffset + 2] ??
            255) /
            255) *
            edgeGain,
        );
